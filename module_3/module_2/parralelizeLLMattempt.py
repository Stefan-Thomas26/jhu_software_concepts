...
def process_chunk(chunk, chunk_id):
    ...
    for row in chunk:
        output.append(enrich_row(row))


    out_file = f"outputData/out_{chunk_id}.json"

    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"Finished chunk {chunk_id} → {len(output)} rows")
...
applicantDataFilename = "outputData/applicant_data.json"
...
# ========================================
numLlmWorkers = min(4, cpu_count()) #starting small

# Load Data using default .json file viewer on machine
applicantDataRows = loadData.load_data(applicantDataFilePath.resolve())

chunks = chunkify(applicantDataRows, numLlmWorkers)

processes = []

for ii, chunk in enumerate(chunks):
    p = Process(target=process_chunk, args=(chunk, ii))
    p.start()
    processes.append(p)
...
for file in glob.glob("output/out_*.json"):
...
print(f"Merged {len(all_data)} rows")

print("All processes finished")


# # run thread pool
# with (max_workers = numLlmWorkers) as executor:
#     enriched_rows = list(executor.map(enrich_row, applicantDataRows))



outputLLMfilename = "outputData/llm_extended_applicant_data.json"
